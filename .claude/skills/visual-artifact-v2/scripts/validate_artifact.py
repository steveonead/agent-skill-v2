#!/usr/bin/env python3
"""Validate structural contracts for a visual-artifact-v2 HTML file."""
...
def validate_diffs(root: Node, source: str, errors: list[str]) -> None:
    diffs = [node for node in root.descendants() if "data-diff" in node.attrs]
    if not diffs:
        return

    required_markers = {
        "Pierre import": "from 'https://esm.sh/@pierre/diffs@1'",
        "Pierre renderer": "new FileDiff(",
        "before/after parser": "parseDiffFromFile(",
        "patch parser": "parsePatchFiles(",
        "Diff selector": "querySelectorAll('[data-diff]')",
        "fallback success gate": ".diff-view.is-rendered .diff-fallback",
    }
    for name, marker in required_markers.items():
        if marker not in source:
            errors.append(f"Diff components require the template {name}")

    for diff in diffs:
        hosts = [node for node in diff.descendants() if "data-diff-host" in node.attrs]
        fallbacks = [node for node in diff.descendants() if "diff-fallback" in node.classes()]
        before = [node for node in diff.descendants() if "data-diff-before" in node.attrs]
        after = [node for node in diff.descendants() if "data-diff-after" in node.attrs]
        patches = [node for node in diff.descendants() if "data-diff-patch" in node.attrs]

        if len(hosts) != 1:
            errors.append(f"line {diff.line}: data-diff requires exactly one data-diff-host")
        if len(fallbacks) != 1:
            errors.append(f"line {diff.line}: data-diff requires exactly one .diff-fallback")

        has_pair = len(before) == 1 and len(after) == 1 and not patches
        has_patch = len(patches) == 1 and not before and not after
        if not has_pair and not has_patch:
            errors.append(
                f"line {diff.line}: data-diff needs one before/after pair or one patch source, but not both"
            )
        if has_pair and not diff.attrs.get("data-file-name", "").strip():
            errors.append(f"line {diff.line}: before/after Diff requires data-file-name")
        style = diff.attrs.get("data-diff-style", "split")
        if style not in {"split", "unified"}:
            errors.append(f"line {diff.line}: data-diff-style must be 'split' or 'unified'")

        for source_node in [*before, *after, *patches]:
            if source_node.tag != "pre":
                errors.append(f"line {source_node.line}: Diff source must be a pre element")
            if "data-diff-source" not in source_node.attrs:
                errors.append(f"line {source_node.line}: Diff source requires data-diff-source")
            if not source_node.attrs.get("data-lang", "").strip():
                errors.append(f"line {source_node.line}: Diff source requires data-lang")
            if first(source_node.descendants("code")) is None:
                errors.append(f"line {source_node.line}: Diff source requires a code child")
        if has_patch and patches[0].attrs.get("data-lang") != "diff":
            errors.append(f"line {patches[0].line}: patch source data-lang must be 'diff'")
...
def validate_code_renderer(root: Node, source: str, errors: list[str]) -> None:
    code_blocks = [
        ...
        if "mermaid" not in pre.classes()
        and "data-diff-source" not in pre.attrs
        and pre.attrs.get("data-lang", "").strip()
    ]
    if not code_blocks:
        return
    required_markers = {
...
def validate(path: Path) -> list[str]:
    ...
    validate_diffs(root, source, errors)
